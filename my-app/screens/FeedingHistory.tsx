...
export default function FeedingHistory() {
  ...
  const load = useCallback(async () => {
    const { data, error } = await supabase.from('feeding_events').select('id, created_at, portions').order('created_at', { ascending: false }).limit(100);
    setList(error ? [] : (data || []));
  }, []);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <Text style={[styles.title, { color: theme.text }]}>Feeding history</Text>
      <FlatList
        data={list}
        ...
        ListEmptyComponent={<Text style={[styles.muted, { color: theme.textMuted }]}>No feeding events yet. Data will appear when the feeder runs.</Text>}
        renderItem={({ item }) => (
          <View style={[styles.row, { borderBottomColor: theme.border }]}>
            <Text style={[styles.portions, { color: theme.text }]}>{item.portions} portion(s)</Text>
...
const styles = StyleSheet.create({
  ...
  portions: { fontSize: 16, fontWeight: '500' },
  date: { fontSize: 14 },
});
